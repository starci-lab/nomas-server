...
@Injectable()
export class GraphQLSignatureStrategy extends PassportStrategy(Strategy, GRAPHQL_SIGNATURE_STRATEGY) {
    constructor(
        private readonly authBlockchainService: AuthService,
    ...
    async validate(req: Request): Promise<void> {
        try {
            ...
            // verify signature
            const isValid = await this.authBlockchainService.verify({
                publicKey,
                accountAddress,
                message,
                signature,
